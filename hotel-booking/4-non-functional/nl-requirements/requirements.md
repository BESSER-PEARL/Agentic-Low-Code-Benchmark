...
The total price of the booking is likewise not typed in. It is worked out from the agreed prices of the rooms involved, the length of the stay and any extra charges recorded against those rooms.
### Things a booking can do
A booking offers five actions, each of which reports back whether it succeeded, since each may legitimately be refused depending on the current state of things.
It can produce the bill for the stay. It can register the arrival of the guests, which moves the stay from not-yet-arrived to checked in. It can register their departure, which moves the stay to checked out. It can be cancelled, which puts it into the cancelled state and releases it. Finally, it can compute and return the amount owed, adding up the agreed room prices over the nights booked together with the extra charges recorded.
### Billing
A booking may have a bill raised against it, but never more than one, and it may go without one entirely. A bill always belongs to exactly one booking.
Each bill carries its own identifying number, the date it was issued, the total amount due, and an indication of whether it has been settled. A bill offers a single action: registering its payment, which reports back whether the payment went through. Settling the bill is what moves the corresponding booking from awaiting payment to confirmed.

### Interface
The application interface must be visually appealing, using a modern style with an orange color scheme and clear typography to present rooms and booking information in an engaging way. It must support three languages, English, French and Luxembourgish, so that users can interact with the system in their preferred language, with all labels, messages and content fully translated. It must also be accessible, ensuring sufficient color contrast, readable font sizes, and keyboard navigability so that users with visual or motor impairments can navigate and use the application fully.
